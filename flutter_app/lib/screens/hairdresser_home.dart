...
// --- CHANGED to ConsumerStatefulWidget to access initState ---
class HairdresserHomeScreen extends ConsumerStatefulWidget {
  final String token;
  ...
  @override
  ConsumerState<HairdresserHomeScreen> createState() => _HairdresserHomeScreenState();
}

// --- ADDED a corresponding ConsumerState class ---
class _HairdresserHomeScreenState extends ConsumerState<HairdresserHomeScreen> {

  // --- THIS IS THE FIX ---
  // We use initState to call our fetchData method once when the screen loads.
  @override
  void initState() {
    super.initState();
    // Use addPostFrameCallback to ensure the widget is built before calling this.
    WidgetsBinding.instance.addPostFrameCallback((_) {
      // Use ref.read here because we are in a lifecycle method.
      ref.read(hairdresserHomeProvider.notifier).fetchData(token: widget.token);
    });
  }

  @override
  Widget build(BuildContext context) {
    // We now watch the provider from inside the build method.
    final viewModel = ref.watch(hairdresserHomeProvider);
      ...
      body: Column(
        ...
        children: [
            ...
            child: viewModel.posts.isEmpty
              ...
              itemBuilder: (context, index) {
                ...
                return _PostCard(
                  ...
                  // Use widget.onProfileClick to access the callback
                  onVisitProfile: () => widget.onProfileClick(post.userId.toString()),
                );
              },
            ),
          ),
          Footer(
            ...
            // Use widget.on... to access the callbacks
...
// Reusable widget for the post card (no changes here)
